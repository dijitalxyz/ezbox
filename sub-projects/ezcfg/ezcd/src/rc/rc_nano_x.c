...
int rc_nano_x(int flag)
{
	...
	case RC_START :
		...
		/* nano-X MOUSE_PORT */
		device[0] = '\0';
		rc = ezcfg_api_nvram_get(NVRAM_SERVICE_OPTION(NANO_X, MOUSE_PORT), device, sizeof(device));
		if (device[0] != '\0') {
			setenv("MOUSE_PORT", device, 1);
		}

		/* nano-X CONSOLE */
		device[0] = '\0';
		rc = ezcfg_api_nvram_get(NVRAM_SERVICE_OPTION(NANO_X, CONSOLE), device, sizeof(device));
		if (device[0] != '\0') {
			setenv("CONSOLE", device, 1);
		}
