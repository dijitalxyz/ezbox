...
/* Forward decl.s */
static irqreturn_t solos_uart_int(int irq, void *dev_id);
...
static void solos_uart_stop_tx(struct uart_port *port)
{
	/* Turn off Tx interrupts. The port lock is held at this point */
	CHMIO_VIRT(UART_CTRL) &= ~CHARM_UART_TX_INTEN;
}

static void solos_uart_start_tx(struct uart_port *port)
{
...
static int solos_uart_startup(struct uart_port *port)
{
	...
	/* Request IRQ */
	ret = request_irq(IRQ_UART, solos_uart_int, IRQF_DISABLED | IRQF_SAMPLE_RANDOM, "solos_uart", port);
	if (ret) {
		printk(KERN_ERR "could not claim IRQ %u\n", port->irq);
		return ret;
	}
...
{
	solos_uart_stop_tx(port);
	solos_uart_stop_rx(port);
	solos_uart_break_ctl(port, 0);
	free_irq(IRQ_UART, port);
}

static void solos_uart_set_termios(struct uart_port *port, struct ktermios *new,
				   struct ktermios *old)
{
...
irqreturn_t solos_uart_int(int irq, void *dev_id)
{
